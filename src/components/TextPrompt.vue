  ...
  <div class="d-flex justify-content-center flex-column">
    <div class="d-flex flex-row">
      ...
      <div>
        ...
        <div
          class="d-flex flex-column align-items-start bg-black inventory equipment mt-2"
        >
          <div id="equipment-title">Player equipment</div>
          <div v-for="[key, val] of Object.entries(equipment)" :key="key">
            {{ titleCasify(key) }}: {{ val.name }}
          </div>
        </div>
        <div class="d-flex align-items-center bg-black inventory currency mt-2">
...
export default {
  ...
  data() {
    return {
      ...
      input: "", // user input.
      context: null,
      contexts: { explore: null, shop: "shop" },
      activeNpc: null,
  ...
  methods: {
    restart() {
      ...
      this.equipment = this.loadPlayerEquipment();
    ...
    loadPlayerInventory() {
      ...
      const invArr = this.game.startingInventory.map((o) => {
        const item = this.game.items.find((i) => i.id === o.id);
        return { item: item, qty: o.qty };
      });
      return new Inventory(invArr);
    },
    loadPlayerEquipment() {
      const equipment = {};
      for (const [key, val] of Object.entries(this.game.startingEquipment)) {
        console.log([key, val]);
        equipment[key] = this.game.items.find((i) => i.id === val);
      }
      return equipment;
    },
    initLocations() {
    ...
    findItemInInput(text) {
      ...
      let items = this.inventory
        .items()
        .concat(this.location.inventory.items());
      if (this.activeNpc) {
    ...
    parseInput(text) {
      ...
      // check for context
      if (this.context !== this.contexts.explore) {
      ...
      // case: equip item
        ...
        if (inputArr.length < 2) {
          this.addText(
            `Please specify what you'd like to ${this.aliases.equip[0]}.`
          );
      ...
      // case: unequip item
        ...
        if (inputArr.length < 2) {
          this.addText(
            `Please specify what you'd like to ${this.aliases.unequip[0]}.`
          );
    ...
    parseContext(inputArr, first, item) {
      ...
      if (this.context === this.contexts.shop) this.parseShopInput(first, item);
    },
    parseShopInput(first, item) {
      ...
      if (shopLeaveAliases.includes(first)) {
        this.context = this.contexts.explore;
      ...
      if (
        !item ||
        (!this.inventory.hasItem(item) && !this.activeNpc.items.hasItem(item))
      ) {
        ...
        if (item.price === null) {
          ...
          this.addText(
            `${this.activeNpc.name} rejects your sale offer! It looks like they aren't interested in that ${this.game.aliases.item}.`
          );
        } else {
          this.shopSell(item);
        }
    ...
    shopPurchase(item) {
      if (this.playerCurrency < item.price) {
        this.addText(
          `You don't have enough ${this.game.currencyTerms.generic} to buy ${item.name}.`
        );
        return;
      }
      ...
      this.addText(
        `${this.activeNpc.name}: ${this.getRandomElement(
          this.activeNpc.purchaseLines
        )}`
      );
    },
    shopSell(item) {
      if (this.activeNpc.currency < item.price) {
        this.addText(
          `${this.activeNpc.name} doesn't have enough ${this.game.currencyTerms.generic} to buy ${item.name}.`
        );
        return;
      }
      this.activeNpc.currency -= item.price;
    ...
    getRandomNpcLine(type) {
      this.addText(
        `${this.activeNpc.name}: ${this.getRandomElement(this.activeNpc[type])}`
      );
    },
    handleEquipItem(item) {
      if (!this.inventory.hasItem(item)) {
        ...
        ![this.itemType.armor, this.itemType.weapon].includes(item.type)
      ) {
        this.addText(
          `You can only ${this.aliases.equip} ${this.aliases.armor} and ${this.aliases.weapon} type items.`
        );
      } else {
    ...
    shop(npc) {
      ...
      this.context = this.contexts.shop;
      this.activeNpc = npc;
    ...
    displayShopItems() {
      this.addText(
        `${this.activeNpc.name}'s ${this.game.currencyTerms.generic}: ${this.activeNpc.currency}`
      );
    ...
    resolveItemInLocation(itemId, condition) {
      ...
      if (condition.newLocation !== null) {
        ...
        this.locations[condition.location] =
          this.locations[condition.newLocation];
      }
    ...
    help(context) {
      const helpMsg = [];
      const a = this.game.aliases;

      if (context === this.contexts.shop) {
        helpMsg.push(`--- Help: ${a.shop} context ---`);
        helpMsg.push(
          `You are currently interacting with ${this.activeNpc.name}.`
        );
        helpMsg.push(
          `${this.titleCasify(a.item)}s for sale are listed in the ${
            a.shop
          } menu.`
        );
        helpMsg.push(
          `To ${a.shopBuy[0]} ${this.aOrAn(a.item)} ${a.item}, try entering '${
            a.shopBuy[0]
          } <${a.item} name>'.`
        );
        helpMsg.push(
          `To ${a.shopSell[0]} ${this.aOrAn(a.item)} ${
            a.item
          }, use the same syntax using the command '${a.shopSell[0]}'.`
        );
        helpMsg.push(
          `To leave the shop (and gain access to normal commands), try entering '${a.shopLeave[0]}'.`
        );
      } else if (context === this.contexts.explore) {
        helpMsg.push(`--- Help: ${a.explore} context ---`);
        helpMsg.push(
          `You are playing a role playing text adventure! In this type of game, you use text commands to interact with the game.`
        );
        helpMsg.push(
          `You get information about the game through text descriptions. Use <${a.look[0]}> to get the description of your current ${a.location}.`
        );
        helpMsg.push(
          `Use <${a.examineCommands[0]}> to ${a.examine} ${this.aOrAn(
            a.item
          )} ${a.item}.`
        );
        helpMsg.push(
          `Try other commands like <${a.get[0]}>, <${a.drop[0]}>, <${a.talk[0]}>, <${a.equip[0]}>, or <${a.useItem.food[0]}> to interact with the game.`
        );
        helpMsg.push(
          `There are other commands. Be creative, have fun, and try to figure out how to beat the game!`
        );
      }

      helpMsg.push("--- End help ---");
      this.addText(helpMsg.join("\n"));
    },
    aOrAn(word) {
